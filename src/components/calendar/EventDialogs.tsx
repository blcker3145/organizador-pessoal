...
import { Checkbox, Modal, useEscape } from "../common";
  ...
  return (
    <Modal
      ...
      footer={
        <>
          {editing && onDelete && (
            <button className="btn danger" style={{ marginRight: "auto" }} onClick={onDelete} disabled={saving}>
      ...
      >
        ...
        <div className="ee-row">
          ...
          <div className="ee-when">
            ...
            {!d.allDay && <input id="ev-start-time" className="input" type="time" step={300} value={d.startTime} onChange={(e) => changeStartTime(e.target.value)} aria-label="Hora de início" />}
            <span className="muted">até</span>
            {!d.allDay && <input id="ev-end-time" className="input" type="time" step={300} value={d.endTime} onChange={(e) => set({ endTime: e.target.value })} aria-label="Hora de fim" />}
        ...
        <div className="ee-row">
          ...
          <label className="row" style={{ gap: 6, cursor: "pointer" }}>
            <Checkbox checked={d.allDay} onChange={(v) => set({ allDay: v })} label="Dia inteiro" /> Dia inteiro
          </label>
          {isInstance ? (
            ...
            <select id="ev-repeat" className="select" style={{ width: "auto" }} value={d.repeat} onChange={(e) => set({ repeat: e.target.value as EventDraft["repeat"] })} aria-label="Repetição">
              {d.repeat === "custom" && <option value="custom">Personalizada (mantida)</option>}
        ...
        <div className="ee-row">
          ...
          <div className="ee-colors" role="radiogroup" aria-label="Cor">
            <button type="button" className={cx("ee-color", !d.colorId && "on")} style={{ background: calColor }} onClick={() => set({ colorId: "" })} title="Cor da agenda" aria-label="Cor da agenda" />
            {GOOGLE_EVENT_COLORS.map((c) => (
              <button type="button" key={c.id} className={cx("ee-color", d.colorId === c.id && "on")} style={{ background: c.hex }} onClick={() => set({ colorId: c.id })} title={c.name} aria-label={c.name} />
            ))}
          </div>
        </div>

        {!connected && (
          <div className="ai-note" style={{ marginLeft: 26 }}>
            Conecte o Google Agenda para salvar no Google, convidar pessoas e criar reuniões do Meet.{" "}
        ...
        {isGoogle && (
          <div className="ee-row" style={{ alignItems: "flex-start" }}>
            <Users size={16} className="muted" style={{ marginTop: 8 }} />
            <div className="stack grow" style={{ gap: 6 }}>
              <div className="row">
                <input
                  id="ev-guest"
                  className="input"
                  placeholder="Adicionar convidados (e-mail)"
                  value={guest}
                ...
                <div className="row wrap" style={{ gap: 4 }}>
                  {d.guests.map((g) => (
                    <span key={g} className="pill">
                      {g}
                      <button type="button" className="link-btn" aria-label={`Remover ${g}`} onClick={() => set({ guests: d.guests.filter((x) => x !== g) })}>
                        <X size={11} />
                      </button>
                    </span>
                  ))}
              ...
              {d.guests.length > 0 && (
                <label className="row" style={{ gap: 6, fontSize: 13, cursor: "pointer" }}>
                  <Checkbox checked={d.sendInvites} onChange={(v) => set({ sendInvites: v })} label="Enviar convite por e-mail" /> Enviar convite e atualizações por e-mail
                </label>
              )}
            </div>
          </div>
        ...
        {isGoogle && (
          <div className="ee-row">
            ...
            <label className="row" style={{ gap: 6, cursor: "pointer" }}>
              <Checkbox checked={d.addMeet} onChange={(v) => set({ addMeet: v })} label="Google Meet" /> Adicionar videoconferência do Google Meet
            </label>
          </div>
        )}
        ...
        <div className="ee-row">
          ...
          <input id="ev-location" className="input" placeholder="Adicionar local" value={d.location} onChange={(e) => set({ location: e.target.value })} />
        </div>

        <div className="ee-row" style={{ alignItems: "flex-start" }}>
          <Bell size={16} className="muted" style={{ marginTop: 8 }} />
          <div className="stack grow" style={{ gap: 6 }}>
            {isGoogle && (
              <label className="row" style={{ gap: 6, fontSize: 13, cursor: "pointer" }}>
                <Checkbox checked={d.useDefaultReminders} onChange={(v) => set({ useDefaultReminders: v })} label="Notificações padrão" /> Usar as notificações padrão da agenda
              </label>
            )}
              ...
              <>
                {d.reminders.map((m, i) => (
                  <div key={i} className="row">
                    ...
                    <button type="button" className="icon-btn" aria-label="Remover notificação" onClick={() => set({ reminders: d.reminders.filter((_, j) => j !== i) })}>
                      <X size={14} />
                    </button>
                  </div>
                ))}
                {d.reminders.length < 5 && (
                  <button type="button" className="link-btn" onClick={() => set({ reminders: [...d.reminders, 10] })}>
                    <Plus size={13} /> Adicionar notificação
                  </button>
                )}
                {!isGoogle && d.reminders.length > 0 && (
                  <span className="muted" style={{ fontSize: 12 }}>
                    Eventos só do Organizador não enviam notificações. Salve no Google Agenda para ser avisado.
                  </span>
                )}
              </>
            )}
        ...
        {isGoogle && (
          <div className="ee-row">
            ...
            <select id="ev-transparency" className="select" style={{ width: "auto" }} value={d.transparency} onChange={(e) => set({ transparency: e.target.value as EventDraft["transparency"] })} aria-label="Disponibilidade">
              <option value="opaque">Ocupado</option>
              <option value="transparent">Disponível</option>
            </select>
            <select id="ev-visibility" className="select" style={{ width: "auto" }} value={d.visibility} onChange={(e) => set({ visibility: e.target.value as EventDraft["visibility"] })} aria-label="Visibilidade">
        ...
        <div className="ee-row" style={{ alignItems: "flex-start" }}>
          <AlignLeft size={16} className="muted" style={{ marginTop: 8 }} />
          <textarea id="ev-description" className="textarea" rows={4} placeholder="Adicionar descrição" value={d.description} onChange={(e) => set({ description: e.target.value })} />
        </div>
